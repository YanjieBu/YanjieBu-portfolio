...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const hoverContainer = document.querySelector('.hover-titles');

  // 你的项目名称列表（不含 My Works.）
  const projectTitles = [
    'Health Band',
    'Bne Lions Stats',
    'Brisbane Wildtrack',
  ...
  // 1) 生成所有动态 span，初始都加上 hide 类
  const spans = projectTitles.map(name => {
    const s = document.createElement('span');
    s.className = 'hover-title hide';
    s.textContent = name;
    hoverContainer.appendChild(s);
    return s;
  });

  // 2) 给每个项目封面绑定事件
  document.querySelectorAll('.projects a').forEach((link, idx) => {
